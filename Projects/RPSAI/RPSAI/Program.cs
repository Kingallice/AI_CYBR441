        ...
        static void Main(string[] args)
        {
            ...
            if (File.Exists(FILEPATH) && !blnReset)
            {
                ...
                if (fileIn != "")
                {
                    ...
                    int wNum = Int32.Parse(fileParms[1].Trim().Split(':')[1]);
                    int lL = Int32.Parse(fileParms[2].Trim().Split(':')[1]);
                    int lNum = Int32.Parse(fileParms[3].Trim().Split(':')[1]);
                    int lD = Int32.Parse(fileParms[4].Trim().Split(':')[1]);
                    int dNum = Int32.Parse(fileParms[5].Trim().Split(':')[1]);

                    string ArrPass = fileParms[6].Trim().Split(':')[1].Replace('[', ' ').Replace(']', ' ').Trim();
                    int[] lThree = new int[3];
                    for (int i = 0; i < lThree.Length; i++)
                    {
                        lThree[i] = Int32.Parse(ArrPass.Split(',')[i].Trim());
                    }
                    rpsAI = new AIClass(rNum, wNum, lL, lNum, lD, dNum, lThree);
                }
            }
            else
            {
